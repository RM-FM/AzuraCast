...
setup-release() {
  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/RM-FM/AzuraCast/deploy/sample.env -o .env
  fi
...
run-installer() {
  ...
  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/RM-FM/AzuraCast/$AZURACAST_RELEASE_BRANCH/sample.env -o .env
  fi
  if [[ ! -f azuracast.env ]]; then
    curl -fsSL https://raw.githubusercontent.com/RM-FM/AzuraCast/$AZURACAST_RELEASE_BRANCH/azuracast.sample.env -o azuracast.env
  fi
  if [[ ! -f docker-compose.yml ]]; then
    curl -fsSL https://raw.githubusercontent.com/RM-FM/AzuraCast/$AZURACAST_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.yml
  fi
  ...
  curl -fsSL https://raw.githubusercontent.com/RM-FM/AzuraCast/$AZURACAST_RELEASE_BRANCH/docker-compose.installer.yml -o docker-compose.installer.yml
...
update() {
  ...
  if ask "Are you ready to continue with the update?" Y; then
    ...
    curl -fsSL https://raw.githubusercontent.com/RM-FM/AzuraCast/$AZURACAST_RELEASE_BRANCH/docker.sh -o docker.new.sh
    ...
    if [[ ! -s docker-compose.new.yml ]]; then
      curl -fsSL https://raw.githubusercontent.com/RM-FM/AzuraCast/$AZURACAST_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.new.yml
    fi
...
update-self() {
  ...
  curl -H 'Cache-Control: no-cache, no-store' -fsSL \
    https://raw.githubusercontent.com/RM-FM/AzuraCast/$AZURACAST_RELEASE_BRANCH/docker.sh?$(date +%s) \
    -o docker.sh
  chmod a+x docker.sh
